...
class GameServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        """Handles POST requests by routing them to specific methods based on the endpoint."""
    ...
    def register_player(self, data):
        ...
        if len(username) <= 3 or len(password) <= 3:
            self.send_json_response({"success": False, "message": "Username and password must be at least 4 characters long."})
            return
    ...
    def join_game(self, data):
        ...
        if username in GAMES[game_id]["players"]:
            self.send_json_response({"success": True, "message": f"Already joined game {game_id}."})
            return
    ...
    def c_players_in_game(self, data):
        ...
        self.send_json_response({"success": success, "players": players, "turn": turn})
